...
function SelectTrigger({
  ...
  return (
    <SelectPrimitive.Trigger
      ref={ref}
      className={cn('dark:active:bg-input/50 flex h-10 flex-row items-center justify-between px-3 shadow-black/5 sm:h-9 border-input dark:bg-input/30 dark:hover:bg-input/50 gap-1.5 rounded-md border bg-transparent py-2 pr-2 pl-2.5 text-sm shadow-xs',
        Platform.select({
          ...
          <TextClassContext.Provider value="text-popover-foreground">
            <NativeOnlyAnimatedView className="z-50" entering={FadeIn} exiting={FadeOut}>
              <SelectPrimitive.Content
                className={cn('border-border relative z-50 border shadow-black/5 bg-popover min-w-36 rounded-md shadow-md',
                  Platform.select({
                    web: cn(
...
function SelectLabel({
  ...
  return (
    <SelectPrimitive.Label
      className={cn('sm:py-1.5 text-muted-foreground px-2 py-1.5 text-xs', className)}
      {...props}
    />
  );
}
...
function SelectItem({
  ...
  return (
    <SelectPrimitive.Item
      className={cn('active:bg-accent group relative flex w-full flex-row items-center sm:py-1.5 gap-2 rounded-sm py-1.5 pr-8 pl-2 text-sm',
        Platform.select({
