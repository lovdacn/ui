...
const buttonVariants = cva(cn(
    'group shrink-0 flex-row items-center justify-center gap-2 shadow-none rounded-md border border-transparent',
    Platform.select({
  ...
  ),
  {
    variants: {
      variant: {
        ...
        destructive: cn('active:bg-destructive/90 shadow-sm shadow-black/5 bg-destructive/10 hover:bg-destructive/20 dark:bg-destructive/20 dark:hover:bg-destructive/30',
          Platform.select({
            web: 'hover:bg-destructive/90 focus-visible:ring-destructive/20 dark:focus-visible:ring-destructive/40',
          })
        ),
        outline: cn('active:bg-accent dark:active:bg-input/50 border shadow-black/5 border-border bg-background hover:bg-muted dark:bg-input/30 dark:border-input dark:hover:bg-input/50 aria-expanded:bg-muted shadow-xs',
          Platform.select({
      ...
      size: {
        default: cn('h-10 py-2 sm:h-9 min-h-9 gap-1.5 px-2.5', Platform.select({ web: 'has-[>svg]:px-3' })),
        sm: cn('h-9 rounded-md sm:h-8 min-h-8 gap-1 px-2.5', Platform.select({ web: 'has-[>svg]:px-2.5' })),
        lg: cn('h-11 rounded-md sm:h-10 min-h-10 gap-1.5 px-2.5', Platform.select({ web: 'has-[>svg]:px-4' })),
