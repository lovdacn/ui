...
/**
 * `React.ComponentProps` deliberately omits `ref`, so a host typed with it rejects
 * every `ref={...}` a component forwards — which is what broke Carousel, Spinner
 * and InputOTP. `ComponentPropsWithRef` keeps it, and `React.ElementType` is the
 * constraint that alias actually declares. Leaving `C` unconstrained instead makes
 * the whole alias fail to resolve, degrading every host to `{}` and surfacing as
 * "children does not exist" on Text, View and everything built on them.
 */
type PlainProps<C extends React.ElementType> = React.ComponentPropsWithRef<C> &
