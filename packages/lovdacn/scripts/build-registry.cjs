...
const LEGACY_STYLE_LABELS = { default: 'Default', 'new-york': 'New York' };
const LEGACY_STYLES = Object.entries(DESIGN_ALIASES.styles).map(([name, alias]) => ({
  name,
  label: LEGACY_STYLE_LABELS[name] || name,
  description: alias.reason,
  recipe: alias.to,
  legacy: true,
}));
const STYLES = [...ACTIVE_STYLES, ...LEGACY_STYLES];
...
function parseImports(content, componentName) {
  ...
  while ((match = importRegex.exec(content)) !== null) {
    ...
    // Generated by init/add/apply from the selected static font manifest.
    else if (source.includes('lib/lvcn-fonts')) {
      // Project-owned resource; no registry or npm dependency.
    }
 ...
 */
const extractTextClasses = recipeCompiler.extractTextClasses

const extractContainerClasses = recipeCompiler.extractContainerClasses
...
const isNativeHostileClass = recipeCompiler.isNativeHostileClass
...
function buildEngine(engine) {
  ...
  for (const style of STYLES) {
    ...
    const styleCssPath = path.join(DESIGN_ROOT, 'styles', `style-${style.recipe}.css`);
    let styleMap = {};
...
function copyCanonicalCssFiles() {
  for (const file of fs.readdirSync(DEST_REGISTRY)) {
    if (/^style-[\w-]+\.css$/.test(file)) {
      fs.removeSync(path.join(DEST_REGISTRY, file));
    }
  }

  for (const style of ACTIVE_STYLES) {
    const source = path.join(DESIGN_ROOT, 'styles', `style-${style.name}.css`);
    const destination = path.join(DEST_REGISTRY, `style-${style.name}.css`);
    fs.copyFileSync(source, destination);
  }
  console.log(`✔  Copied ${ACTIVE_STYLES.length} canonical style CSS debug files to registry root`);
}
...
fs.writeJsonSync(
  path.join(DEST_REGISTRY, 'index.json'),
  DESIGN_CATALOG.styles.map(({ name, label, description }) => ({ name, label, description })),
  { spaces: 2 }
);

// Refresh canonical CSS debug copies without exposing compatibility aliases as active styles.
copyCanonicalCssFiles();
