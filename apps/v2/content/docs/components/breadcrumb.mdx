...
<Callout className="border-amber-500/30 bg-amber-500/10 text-amber-900 dark:text-amber-200">
  **In active development for Expo Web.** Because Expo apps also run on the web,
  `breadcrumb` is still being refined for web layouts (hover states, truncation,
  and keyboard focus). It works on native today, but the web styling and API may
  still change.
</Callout>
