...
- **Tablet & desktop (`≥ 768px`)** — the sidebar is an in-flow column that animates its width when toggled (`offcanvas` hides it; `icon` collapses to a narrow icon rail).
...
<Callout>
  ...
  **only its icon** — labels, `SidebarMenuAction`s and `SidebarMenuBadge`s are hidden —
...
- **Keep labels on touch.** Reserve `collapsible="icon"` for **tablets and desktop** (use carefully on mobile devices) — on phones, icon-only targets are ambiguous without a tooltip, so prefer full-width rows with a label.
