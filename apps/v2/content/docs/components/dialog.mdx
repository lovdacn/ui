...
```bash
bunx --bun lovdacn@latest add dialog
```
...
```bash
npm install @rn-primitives/dialog@^1.5.2 lucide-react-native @rn-primitives/portal@^1.5.2
```

<Step>Add the registry components this component depends on: `icon`, `native-only-animated-view`, `utils`.</Step>
